...
WHERE
    user_id = 'U003'àng có  order_id = 3 bị huỷ, Hãy viết câu lệnh cập nhật order_status thành "Cancelled".
UPDATE order_hk 
SET 
...
SELECT 
    p.product_id, p.product_name, od.order_id
FROM
    product p
        LEFT JOIN
...
SELECT 
    order_status, COUNT(order_id) AS Total_Order
FROM
    order_hk
...
FROM order_hk
WHERE total_price > (SELECT AVG(total_price) FROM order_hk);
...
FROM
    user u
        JOIN
    order_hk o ON u.user_id = o.user_id
        JOIN
    order_detail od ON o.order_id = od.order_id
        JOIN
    product p ON od.product_id = p.product_id
WHERE
    p.product_name = 'Giay sneaker';
